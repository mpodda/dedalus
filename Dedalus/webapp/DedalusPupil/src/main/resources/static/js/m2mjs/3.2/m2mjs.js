...
class Connection {
	...
	static post (url, requestData, isJson = true) {
		return new Connection(url, Connection.Methods.POST, requestData, isJson).connect();
	}
    ...
    static getElement(parentElement, elementId) {
        ...
        for (var i = 0; i < parentElement.childNodes.length; i++) {
            ...
            if (returnElement == null && parentElement.childNodes[i].childNodes.length > 0) {
                var element = XBrowser.getElement(parentElement.childNodes[i], elementId);
                if (element != null) {
                    return element;
                }
            }
                    ...
                    if (field.hasDataType() && hasValue){
                        switch (field.dataType) {
                            case Validator.DataTypes.EMAIL:
                                ...
                                if (!isEmail) {
                                    field.currentErrorMessage = Validator.resolveErrorMessage(field.datatypeViolationErrorMessage);
                        ...
                        externalValidationFunction.fields.forEach (
                            async (field, index) => {
                                if (isValid) {
                                    ...
                                    field.currentErrorMessage = Validator.resolveErrorMessage(externalValidationFunction.errorMessage);
...
class GridRow {
    constructor (gridTemplate, modelObject, objectPaths, renderers, components, index) {
    ...
    renderGridRow() {
        ...
        this.renderers.forEach (
            async function (renderer, index) {
                const element = XBrowser.getElement(gridRow.gridRowContainer, renderer["name"]);
                if (element) {
        ...
        XBrowser.addHandler(this.gridRowContainer, "mouseover", function onMouseOver(e) {
        ...
        XBrowser.addHandler(this.gridRowContainer, "mouseout", function onMouseOut() {
            if (!gridRow.isSelected) {
    ...
    static dataComparator (object1, object2, sort) {
        ...
        switch (sort.currentSortedColumn.sortDataType) {
            case Grid.Sort.SortDataTypes.numeric:
                if (sort.currentSortedColumn.sortType == Grid.Sort.SortTypes.ascending) {
            ...
            case Grid.Sort.SortDataTypes.alphanumeric:
                if (resolveObjectValue(sort.currentSortedColumn.dataProperty, object2) > resolveObjectValue(sort.currentSortedColumn.dataProperty, object1)) {
                    if (sort.currentSortedColumn.sortType ==  Grid.Sort.SortTypes.ascending) {
                        return 1;
                    } else {
                        return -1;
            ...
            case Grid.Sort.SortDataTypes.date:
                ...
                value1 = Grid.Sort.parseFromString(date1, sort.currentSortedColumn.dateFormat, resolveObjectValue(sort.currentSortedColumn.dataProperty, object1));
                ...
                value2 = Grid.Sort.parseFromString(date2, sort.currentSortedColumn.dateFormat, resolveObjectValue(sort.currentSortedColumn.dataProperty, object1));
    ...
    renderGrid() {
        ...
        this.getData().forEach (
            function (modelObject, index) {
                let gridRow = new GridRow (grid.gridTemplate, modelObject, grid.objectPaths, grid.renderersSet, grid.componentsSet, index);
